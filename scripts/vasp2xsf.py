...
#output_file=open(outfile, mode='w')
...
for file in sorted(outcar_files):                                   #loop over the list of outcar files
            atoms=read(file, index=':', format="vasp-out")  #load OUTCAR file using ASE module
            #write the configs.xsf files
            for count in range(len(atoms)):
                energy = atoms[count].get_potential_energy()
                outname=os.path.join(outfile, "{}.xsf".format(str(numfiles).zfill(5)))
